...
<div class="page-heading" style="display: flex; justify-content: space-between; align-items: center;">
    <div>
        ...
        <p>Seluruh dokumen yang sudah melewati meja verifikasi ppk.</p>
    </div>
    <div style="display: flex; gap: 10px;">
        <a href="{{ route('ppk.riwayat.export.csv') }}" class="btn-primary" style="background-color: #10b981; text-decoration: none; display: flex; align-items: center; gap: 5px; font-size: 0.875rem;">
            <ion-icon name="document-text-outline"></ion-icon> Unduh Excel
        </a>
        <a href="{{ route('ppk.riwayat.export.pdf') }}" class="btn-primary" style="background-color: #ef4444; text-decoration: none; display: flex; align-items: center; gap: 5px; font-size: 0.875rem;">
...
<div class="panel" style="margin-bottom: 20px; padding: 20px;">
    <form action="{{ route('ppk.riwayat.index') }}" method="GET" style="display: flex; gap: 15px; align-items: flex-end; flex-wrap: wrap;">
        ...
        <div>
            <a href="{{ route('ppk.riwayat.index') }}" style="display: inline-block; padding: 8px 16px; font-size: 12px; color: #ef4444; text-decoration: none; font-weight: 600; border: 1px solid #fee2e2; border-radius: 8px; background: #fef2f2;">Reset</a>
        </div>
        @endif
    </form>
</div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="{{ route('ppk.spj.show', $spj->id) }}" style="color: #2563eb; font-weight: 600; text-decoration: none;">{{ $spj->nomor_spj }}</a></td>
                    ...
                    <td>
                        @if($spj->status == 'revisi_PPK' && $spj->catatan_revisi)
                    ...
                    <td>
                        <form action="{{ route('ppk.riwayat.destroy', $spj->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus permanen riwayat pengajuan ini?');" style="display:inline;">
                            @csrf
                            @method('DELETE')
