...
<div class="panel" style="margin-bottom: 20px; padding: 20px;">
    <form action="{{ route('ppspm.spj.index') }}" method="GET" style="display: flex; gap: 15px; align-items: flex-end; flex-wrap: wrap;">
        ...
        <div>
            <a href="{{ route('ppspm.spj.index') }}" style="display: inline-block; padding: 8px 16px; font-size: 12px; color: #ef4444; text-decoration: none; font-weight: 600; border: 1px solid #fee2e2; border-radius: 8px; background: #fef2f2;">Reset</a>
        </div>
        @endif
    </form>
</div>
...
<div class="panel">
    ...
    <div style="overflow-x: auto;">
        <form id="bulkVerifyForm" action="{{ route('ppspm.spj.bulk-verify') }}" method="POST">
            @csrf
            <table class="table">
                <thead>
                    <tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @php
                            ...
                            if($spj->status == 'disetujui_ppspm') {
                                $badgeClass = 'pending';
                                $statusText = 'Menunggu ppspm';
                            } elseif($spj->status == 'revisi_ppspm') {
                                $badgeClass = 'returned';
                                $statusText = 'Dikembalikan';
                            } elseif(in_array($spj->status, ['disetujui_ppspm', 'disetujui_ppspm', 'selesai'])) {
                    ...
                    <td>
                        <a href="{{ route('ppspm.spj.show', $spj->id) }}" class="btn-primary" style="padding: 0.25rem 0.75rem; font-size: 0.875rem; text-decoration: none; {{ $spj->status == 'disetujui_ppspm' ? 'background-color: #8b5cf6;' : 'background-color: #6b7280;' }}">
                            {{ $spj->status == 'disetujui_ppspm' ? 'Periksa' : 'Detail' }}
                        </a>
                    </td>
                </tr>
                @empty
