...
@section('content')
@push('styles')
    @vite(['resources/css/ppspm/profil.css'])
@endpush


<div class="page-heading">
    <div>
        <h2>Pengaturan Profil ppspm</h2>
...
<div class="panel">
    ...
    <div style="padding: 1.5rem;">
        <form action="{{ route('ppspm.profil.update') }}" method="POST">
            @csrf
            @method('PUT')
            
            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #374151;">Nama Lengkap</label>
                <input type="text" name="name" required style="width: 100%; max-width: 400px; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; outline: none; font-family: inherit;" value="{{ old('name', $user->name) }}">
            </div>

            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #374151;">Alamat Email</label>
                <input type="email" name="email" required style="width: 100%; max-width: 400px; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; outline: none; font-family: inherit;" value="{{ old('email', $user->email) }}">
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="password" name="current_password" style="width: 100%; max-width: 400px; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; outline: none; font-family: inherit;" autocomplete="current-password">
            </div>

            <div style="display: flex; gap: 1.5rem; margin-bottom: 1.5rem;">
                <div style="flex: 1; max-width: 400px;">
                    <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #374151;">Kata Sandi Baru</label>
                    <input type="password" name="password" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; outline: none; font-family: inherit;" autocomplete="new-password">
                </div>
                <div style="flex: 1; max-width: 400px;">
                    <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #374151;">Konfirmasi Kata Sandi</label>
                    <input type="password" name="password_confirmation" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; outline: none; font-family: inherit;" autocomplete="new-password">
                </div>
            </div>
